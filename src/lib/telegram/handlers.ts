...
${md.inlineCode('/help')} - ${i18next.t('Print help messages')}
	...
	sidHandler = async (message: telegram.Message) => {
		...
		const sidStr = message.text?.slice(4).trim(); // "/sid"

		const sid = parseInt(sidStr || '');
		if (isNaN(sid)) return { text: md.build(md`${i18next.t('The server id is not valid.')}`) };
	...
	serverHandler = async (message: telegram.Message) => {
		...
		const serverName = message.text?.slice(7).trim(); // "/server"
		if (!serverName) {
	...
	overviewHandler = async (message: telegram.Message) => {
		...
		const groupName = message.text?.slice(9).trim(); // "/overview"
	...
	monitorHandler = async (message: telegram.Message) => {
		...
		const serviceName = message.text?.slice(8).trim(); // "/monitor"
		if (!serviceName) return { text: md.build(md`${i18next.t('The service name is not valid.')}`) };
	...
	transferHandler = async (message: telegram.Message) => {
		...
		const serviceName = message.text?.slice(9).trim(); // "/transfer"
		if (!serviceName) return { text: md.build(md`${i18next.t('The service name is not valid.')}`) };
	...
	private async generateTransferResponse(serviceName: string) {
		...
		if (!serviceResp.cycle_transfer_stats) return { text: md.build(md`${i18next.t('No cycle transfer data available.')}`) };

		let matchedServiceName: string | undefined;
		const matchedService = Object.values(serviceResp.cycle_transfer_stats).find((s) =>
...
${i18next.t('Next Update Time')}: ${new Date(service.next_update[sid]).toLocaleString(i18next.t('locale'))}
	...
	private async generateMonitorResponse(serviceName: string) {
		const serviceResp = await this.nzClient.getService();
		if (!serviceResp.services) return { text: md.build(md`${i18next.t('No service data available.')}`) };

		let matchedServiceName: string | undefined;

		const matchedService = Object.values(serviceResp.services).find((s) =>
			s.service_name.toLowerCase().includes(serviceName.toLowerCase()),
		);
...
${i18next.t('Last Updated At')}: ${md.bold(new Date().toLocaleString(i18next.t('locale')))}
	...
	private async generateOverviewResponse(groupName?: string) {
		...
		const inlineKeyboard: telegram.InlineKeyboardButton[][] = [
			...
			...groups.reduce((result: telegram.InlineKeyboardButton[][], group, index) => {
...
${i18next.t('Memory')}: ${nezhaUtils.formatUsage(stats.mem_used, stats.mem_total)}% ${nezhaUtils.formatBytes(
				stats.mem_used,
			)}/${nezhaUtils.formatBytes(stats.mem_total)}
${i18next.t('Swap')}: ${nezhaUtils.formatUsage(stats.swap_used, stats.swap_total)}% ${nezhaUtils.formatBytes(
				stats.swap_used,
			)}/${nezhaUtils.formatBytes(stats.swap_total)}
${i18next.t('Disk')}: ${nezhaUtils.formatUsage(stats.disk_used, stats.disk_total)}% ${nezhaUtils.formatBytes(
				stats.disk_used,
			)}/${nezhaUtils.formatBytes(stats.disk_total)}
...
${i18next.t('Last Updated At')}: ${md.bold(new Date().toLocaleString(i18next.t('locale')))}
	...
	private async generateServerResponse(sid: number | string) {
		const sidIsNumber = typeof sid === 'number';
		const servers = await this.nzClient.getServerStats(sidIsNumber ? sid : undefined);
		...
		const response: telegram.CommonResponse = server
			...
			: { text: md.build(md`${i18next.t('The server id is not valid. Please check your prompt again.')}`) };
	...
	private formatServerMessage(server: nezha.ModelServer) {
		...
		return md.build(md`
${getFlagEmoji(server.geoip.country_code)} ${md.bold(server.name)} ${
			nezhaUtils.isOffline(server.last_active) ? `❌️ ${offlineStr}` : `✅ ${onlineStr}`
		}
==========================
...
${i18next.t('Memory')}: ${nezhaUtils.formatUsage(server.state.mem_used, server.host.mem_total)}% ${nezhaUtils.formatBytes(
		...
		)}/${nezhaUtils.formatBytes(server.host.mem_total)}
${i18next.t('Swap')}: ${nezhaUtils.formatUsage(server.state.swap_used, server.host.swap_total)}% ${nezhaUtils.formatBytes(
			server.state.swap_used,
		)}/${nezhaUtils.formatBytes(server.host.swap_total)}
${i18next.t('Disk')}: ${nezhaUtils.formatUsage(server.state.disk_used, server.host.disk_total)}% ${nezhaUtils.formatBytes(
			server.state.disk_used,
		)}/${nezhaUtils.formatBytes(server.host.disk_total)}
...
${i18next.t('Last Updated At')}: ${md.bold(new Date().toLocaleString(i18next.t('locale')))}
	...
	private authHandler = (message: telegram.Message) => {
		...
		if (message.from?.id !== this.uid) return false;
